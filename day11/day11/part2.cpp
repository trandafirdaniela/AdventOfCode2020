...
bool limita(int i, int j) {
    if (0 <= i && i < scaune.size())
        if (0 <= j && j < scaune[0].size())
            return true;
    return false;
}
int ocupat(int r, int c) {
    int n = 0, linie, coloana;
    for (int i = 0; i < 8; i++) {
        // resetare poz
        linie = r;
        coloana = c;
        while (limita(linie, coloana)) {
            if ((linie != r || coloana != c) && scaune[linie][coloana] == 'L')
                break;
            if ((linie != r || coloana != c) && scaune[linie][coloana] == '#') {
                n++;
                break;
            }
            linie += dr[i];
